---
title: 'Checking UI data'
author: "TDC Team + Dannia Guzman + Zarni Htet"
...
This is merely a set up, but this need to be __loaded before__ we can dynamically add in the project start date quarter etc.

* An R library is a collection of functions written by other developers and approved by the open source community for wider usage.
* The function _source()_ allows us to import our custom written functions. The custom functions are written and saved as .R files.   
...
#enter first quarter expected
firstq <-"Q1, 2017"

#enter last quarter expected
lastq <-"Q2, 2017"

#enter the purpose of the program

purp1 <-"resolves QC issues in a UI wage data file received in July 2017. 
This file contains data from Q1 and Q2 2017. 
The program then combines this file with an earlier processed data file 
from April 2017, which includes incomplete data from Q1 2017. "
purp2 <-"This example uses synthetic UI data created for instructional purposes. "
purp3 <-"Please report any bugs and share any improvements by emailing richard.hendra@mdrc.org" 

#enter the number of people for whom UI data were requested
request <- 190

#record an expectation about the match rate (best guess is fine)
expectedmatch <- 40

# Loading custom R functions
# The file "functions.R" can be downloaded from the TDC-UI GitHub under the data_quality_checks_folder
# Insert the file path to the folder where the file "functions.R" is saved in your environment:
source("[YOUR FILE PATH GOES HERE]/functions.R")

# Directory 
# Insert the file path to the folder where the UI wage records data are stored in your environment:
usedir1 <- "[YOUR FILE PATH GOES HERE]"

# Data
# This is a list of the data files we will be using in the program.
# Both of these files can be downloaded from the TDC-UI GitHub under the data_quality_checks_folder
...
# Output directory
# Insert the file path where you would like to save your output in your environment:
savdir1 <- "[YOUR FILE PATH GOES HERE]"
...
# This is the old data file submitted in April 2017. 
load(file.path(usedir1,usefile1)) 
# This is the new data file submitted in June 2017.
load(file.path(usedir1,usefile2)) 
```

### Standardize the column names

Data file column names come in all forms. Some are capitalized. Some have spaces. Some could accidentally have special characters like ".$*"/. The _clean name_ function below removes those special characters and return lowercase variable names consisting only have _, letters and numbers. It's a simple function to tidy up your variable names for aesthetic as well as ease of use as we pipe variable names through the R "Tidy framework" for data processing later on. This can help you to get ahead of problems when you merge or stack files (as is common practice with administrative data),  
...
What follows is an example of how to use R markdown to automatically write a memo.

__`r project`__ is `r projdesc`  

This memo discusses the UI wage data updates received on __`r max(as.Date(uidata1_july$submission_date, '%m/%d/%Y'))`__ that covers __`r firstq`__ through __`r lastq`__.   
...
# storing all the raw counts, ssn values and total earnings for check table later on.
...
It useful to check whether missing data occur more often than expected in some quarters than in others, outside of typical economic trends (e.g., typically there is an uptick in the fourth quarter of each year as the demand for retail workers increases). If certain quarters experience more missing data than others (outside of economic trends), there might be an underlying data quality issue. Note that the most recent quarter or two on a data extraction will likely have fewer records since there is typically a lag between employers reporting wage data for the quarter and statewide earnings data being available for us. 

Our simulated data only contains two quarters of data and does not have any missingness, but real data may be messier. The checks below can be useful to assess missingness and the spread of records by quarter across longer timespans in real data.
```{r}
...
uidata1_july %>% 
  count(quarter) %>% 
  mutate(quarter = as_factor(quarter)) %>% #this keeps retains the current order for the x-axis.
...
#This plot shows the percent of missings in each variable, broken down by a 
#categorical variable in the dataset (here, we use quarters).
#This can help you see whether missingness occurs in certain quarters and for certain variables
gg_miss_fct(x = uidata1_july, fct = quarter)
```
...
## Does missing data occur systematically, such as by earnings amount? 

Some states may not return data for earnings under a certain threshold (such as $500/quarter), so individuals with low earnings in a particular quarter may not show up on the file. 

```{r}
#check the range of the earnings amount, with particular attention to the minimum value. Are there any earnings under 500? 
...
uidata1_july %>%
  ...
  dplyr::arrange(ssn, empid, quarter, earnings, submission_date) %>%
  # count how many duplicates per row. 
  # dup = 1 means a single observation. n > 1 means multiple same observation.
  dplyr::mutate(dup = n()) %>%
...
uidata1_july <-
  ...
  dplyr::distinct() %>% 
  ungroup() # data is ungrouped to its normal 
# flat structure so that we can continue manipulating at the observation level
```

We should now check to ensure all of the exact duplicates are removed. After the removal procedure, there are `r sum(duplicated(uidata1_july))` exact duplicates. We have removed 5 records with an earnings total of $24,812. Note that this comment is hardcoded. Try to automate the message!       
...
Partial duplicates observations are the same on nearly all columns, but different in 1 or more columns. In the example below, we are checking partial duplicates where the observations have the same SSN, employerID, quarter and submission_date, but differ in earnings. There are __`r sum(duplicated(uidata1_july[c("ssn", "empid", "quarter", "submission_date")]))`__ such partial duplicates in this newly returned file. You may be interested in a assessing variety of partial duplicates, depending on what fields are included in the returned file. With UI earnings data, some common checks might include:
...
In our example here, we are looking at partial duplicates where the observations share the same SSN, employer ID, and quarter, but have different earnings amount.  
...
uidata1_july %>%
  ...
  dplyr::group_by(ssn, empid, quarter, submission_date) %>% 
  dplyr::arrange(ssn, empid, quarter, submission_date)%>%
  dplyr::mutate(dup = n()) %>%
  dplyr::filter(dup > 1)
theme_zebra(flextable(head(uidata1_july_dups, n = nrow(uidata1_july_dups)))) %>%
  autofit()
...
sum(duplicated(uidata1_july[c("ssn", "empid", "quarter", "submission_date")]))
```

After detecting for these types of partial duplicates, you will need to make a decision about how to deal with these duplicates. Upon further investigation or discussion with the UI agency, you may decide that the partial duplicates are all valid earnings records, so you will sum the earnings. Alternately, you may think that duplicated earnings are errors and make a decision to remove one of the records – either by randomly selecting one to keep, or to keeping the higher amount, etc. There are a variety of reasonable decisions you can make to resolve this issue. In this example, we’ve decided to keep the higher earnings value and remove the lower one.
...
# Checking for the number of partial duplication across selected columns.  
# We removed 5 records with an earnings total of $24,812. 
# This note could be automated as well. 
# It turns out the simulation had partial dups and
# it was the exact same set as above. In a real life situation this probably would not happen!
sum(duplicated(uidata1_july[c("ssn", "empid", "quarter", "submission_date")]))
```
...
uidata1_july_outlier_graph <-
  ...
  xlab ("Quarters") +
  ylab ("Earnings") +
  ggtitle ("Distributions of quarterly earnings amounts") +
...
The table below looks at earning outlier observations who have more than $20,000 with the earnings distribution in perspective by quarter. The outlier values are many times that of mean and median earnings.
...
These outliers should be compared to the full sample averages which are produced below. 

```{r outlierSummary02, warning = FALSE, message = FALSE, echo = FALSE}
...
In the table below, we are looking at low outlier (where program participants earn less than $20). We are not imputing any values to them as the number of observations is insignificant to warrant further investigation with data providers and it is possible that some of them may have earned at those rates. Also they are unlikely to affect averages.
...
The code below transforms the file so that it is easier to compare earnings amounts across quarters.
You can begin to see which observations have earnings in both quarter 1 and quarter 2, only quarter 1 or quarter 2.
...
#Eventually more flexible code should be written to generalize from instances. Give it a try!
# From visual observation, for extreme outliers in Q1, use values from a neighboring quarter
# For less extreme outliers in Q2, top code
uidata1_july_wide <-
uidata1_july_wide %>%
...
Below we are converting the wide data to long data again for two reasons.  
...
uidata1_july <-
  ...
  na.omit(uidata1_july) 
tail(uidata1_july, n = 5) %>%
  flextable::flextable() %>%
  theme_zebra() %>%
  autofit() 
...
### Total quarterly earnings outliers by SSN & Quarter
...
uidata1_july_per_ssn_quarter <- uidata1_july %>%
...
In the table below, we look at all the observations that share the same SSN, but multiple employers in the same quarter. We suspect that multiple people maybe using the same SSN to work at multiple different places. As this will likely affect future quarters, we decide to remove this SSN altogether from our analysis sample. Note for the table below we would normally round earnings values.
```{r, warning = FALSE, message = FALSE}
## pulling out the observations with the ssn for >20,000 USD. 
# We would recommend rounding the earnings values. Try it!
uidata1_july %>%
...
As you process administrative data (as we have above), you drop, sum, combine, and transform records. A best practice to ensure that you have not made any errors is to trace the sum of record counts and values (in this case dollars) to make sure that you can account for any drop or increase in counts or sums. We call this "the Law of Conservation of Data" refering to the principal in physics that energy (here data) can be neither be created, nor destroyed (and if it is, we have an error!)
...
# Processed July Info
procJulyCount <- dim(uidata1_july)[1]
procJulySSNs <- length(unique(uidata1_july$ssn))
procJulyEarnings <- sum(uidata1_july$earnings, na.rm = TRUE)
# July Summary Info
...
# outlier_ssn is a vector that holds all shared ssn and we are only 
# having observations without that ssn. 
# Try to add a message recording how much in earnings was lost when deleting this SSN.
```

### Check for partial duplicates that differ only by submission date

Below, we identify partial duplicates in which all variables except submission date are the same.There are __`r sum(duplicated(uidata1_april_july[c("ssn", "empid", "quarter", "earnings")]))`__ sets of these types of partial duplicates. For these duplicates, we keep the most recent record and remove the earlier record. 
...
# same ssn, empid, quarter and earnings with different submission dates
...
uidata1_april_july <-
  uidata1_april_july %>%
  dplyr::group_by(ssn, empid, quarter, earnings) %>%
  dplyr::slice(which.max(as.Date(submission_date, '%m/%d/%Y')))
```
...
uidata1_april_july %>%
  ...
  # filtering those observations with duplicates
  dplyr::filter(dup > 1) %>% 
  # sorting by ssn, employer id, quarter and submission date
  dplyr::arrange(ssn, empid, quarter, submission_date) 
...
```{r, warning = FALSE, message = FALSE}
## Checking for the number of partial duplicates with 
# same ssn, empid, quarter with different earnings and submission dates
sum(duplicated(uidata1_april_july[c("ssn", "empid", "quarter")]))
```

```{r, warning = FALSE, message = FALSE}
# Removing partial duplicates for observations with 
# same ssn, empid and quarter with different earnings and submission dates
uidata1_april_july <-
  uidata1_april_july %>%
...
The table below examines the __SSNs__ and __employer ids__ for the partial duplicates across earning amounts and different submission dates for the duplicate table above. As you can see, we have successfully removed the earnings from the earlier submission date and kept the most recent one. 
...
In this section, we account for all the records that come in as well as those that have been removed to ensure that we have not leaked or inflated the data.
...
head(countSum) %>% 
  dplyr::select(rawAprilJulyCount, dropAprilJuly, procAprJulyCount) %>%
  flextable::flextable() %>%
  theme_zebra() %>%
  autofit()
```
